// Layer 2 — Navigation. Express proxy: routes request -> jiraClient -> testStrategy(groqClient) -> response.
...
app.post('/api/save', (req, res) => {
  try {
    const id = (req.body?.id || 'strategy').trim().replace(/[^A-Za-z0-9_-]/g, '_');
    const markdown = req.body?.markdown || '';
    const dir = path.join(__dirname, 'output');
    fs.mkdirSync(dir, { recursive: true });
    fs.writeFileSync(path.join(dir, `test-strategy-${id}.md`), markdown, 'utf8');
    res.json({ path: `output/test-strategy-${id}.md` });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
