...
import { generatePlan, generateFromContext, savePlan } from '../lib/api.js';
...
export default function Generator({ config, envStatus, goSettings }) {
  ...
  // Filename base: Jira id, or the derived Test Plan ID (e.g. TP-REQ-001), or a fallback.
  const fileBase = () =>
    (mode === 'jira' ? jiraId.trim() : result?.plan?.testPlanId || 'requirement').replace(/[^A-Za-z0-9_-]/g, '_');
  ...
  async function onGenerate(e) {
    ...
    try {
      const data =
        mode === 'jira'
          ? await generatePlan(jiraId.trim(), config)
  ...
  function download() {
    ...
    a.href = url;
    a.download = `test-plan-${fileBase()}.md`;
    a.click();
  ...
  async function saveServer() {
    setError('');
    try {
      const r = await savePlan(fileBase(), result.markdown);
      setSavedPath(r.path);
    } catch (err) {
      setError(err.message);
    }
