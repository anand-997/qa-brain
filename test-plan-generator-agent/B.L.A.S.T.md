...
### Self-Anneal Hook
If a generated plan drops a section, breaks the 22-section count, leaves a P1/P2 reference undefined, or invents data, treat it as a **tool failure**: read the gap, patch the generator/template, re-test, then record the learning in [`architecture/test-plan-template.md`](./architecture/test-plan-template.md) so it never repeats.

> ✅ **In lockstep (2026-06-06):** the runtime app now emits all **22** sections. The native template ([`architecture/test-plan-template.md`](./architecture/test-plan-template.md)), the JSON schema in `CLAUDE.md §3d`, the generator/renderer ([`tools/testPlan.js`](./tools/testPlan.js)), and the formatted view ([`src/components/TestPlanView.jsx`](./src/components/TestPlanView.jsx)) all match this RICE-POT contract. If you change the section set, update all four together (Golden Rule: SOP first).
